...
- ~~__주문 취소__~~ (2021.01.18 완료)
  ...
  - ~~sql~~
      ...
      ```
- ~~__주문 정보 수정__~~ (2021.01.20 완료)
  - ~~jsp : '주문 완료' 상태일 때만, [주문 정보 수정] 버튼 생성.~~
  - ~~sql : 필요한 항목만 `<include>` 해서 삽입될 수 있도록 하기.~~ (항목이 많지 않아 전부 update 되도록 함)
      ```sql
...
- __답글형 문의 게시판__
  - 관리자만 답글 달 수 있게하기.
- ~~__오류 페이지__~~ (2021.01.22 완료)
  - ~~오류 안내 페이지 만들고 연결하기.~~
- __상품 리뷰__
  - 별점 기능
  - 클릭 시 내용이 전부 나오도록
  - 수정 버튼 작성자만 보일 수 있도록
  - admin이 리뷰에 답글을 달 수 있도록
...
- ~~__시간 오류__~~ (2021.01.31 완료)
  - Q. 리눅스 서버를 KST로 설정했는데, 시간이 이상하게 출력됨.
  - A. RDS도 TimeZone 설정을 따로 해줘야 했음.
- __재고 오류__
  - Q. 재고가 50개일 때, 한 번에 60개 주문하면 마이너스(-)로 재고가 변경됨.
